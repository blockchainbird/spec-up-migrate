...
const REPO_CONFIG = {
  ...
  // Starter pack repository (used for package.json template)
  STARTER_PACK_REPO: {
    owner: 'trustoverip',
    name: 'spec-up-t-starter-pack',
    branch: 'main'
  }
};
...
async function updatePackageJson(directory, dryRun) {
  ...
  try {
    ...
    try {
      const starterPackUrl = generateGitHubRawUrl(
        REPO_CONFIG.STARTER_PACK_REPO,
        'package.spec-up-t.json'
      );
      starterPackageTemplate = await fetchJson(starterPackUrl);
