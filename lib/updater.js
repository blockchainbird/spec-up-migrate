 ...
 * @param {boolean} minimal - Whether to return minimal configuration (fewer markdown files, no logos)
 * @returns {Object} - Fallback spec configuration object
 */
function generateFallbackSpecConfig(minimal = false) {
  const baseConfig = {
  ...
  if (!minimal) {
    // Add additional markdown files for full configuration
    baseConfig.markdown_paths.push(
      "example-markup-in-markdown.md",
      "spec-body.md"
    );

    // Add logo and branding assets
    baseConfig.logo = generateGitHubRawUrl(
      REPO_CONFIG.TRUSTOVERIP_SPEC_UP_T_REPO,
      'src/install-from-boilerplate/boilerplate/static/logo.svg'
    );
    baseConfig.logo_link = generateGitHubRepoUrl(REPO_CONFIG.TRUSTOVERIP_SPEC_UP_T_REPO);
    baseConfig.favicon = generateGitHubRawUrl(
      REPO_CONFIG.TRUSTOVERIP_SPEC_UP_T_REPO,
    ...
    // Add test external specs
    baseConfig.external_specs = [
      {
  ...
  // Always add terms-and-definitions-intro.md at the end if not already present
  if (!baseConfig.markdown_paths.includes('terms-and-definitions-intro.md')) {
    baseConfig.markdown_paths.push('terms-and-definitions-intro.md');
  }
...
async function fetchSpecUpTBoilerplate() {
  ...
  try {
    ...
    return {
      "specs": [generateFallbackSpecConfig(false)]
    };
  }
}
...
async function updateSpecsJson(directory, dryRun) {
  ...
  try {
    ...
    // Replace existing specs with boilerplate spec only
    if (boilerplateSpec) {
      specsData.specs = [boilerplateSpec];
      changes.push('Replaced existing specs with Spec-Up-T boilerplate template');
    } else {
      // Fallback: ensure specs array exists and use minimal spec
      if (!Array.isArray(specsData.specs)) {
        specsData.specs = [];
        changes.push('Created specs array');
      }

      // If no boilerplate was fetched, create a minimal spec
      if (specsData.specs.length === 0) {
        const fallbackSpec = generateFallbackSpecConfig(true);
        specsData.specs = [fallbackSpec];
        changes.push('Added fallback Spec-Up-T spec configuration');
      }
    }
