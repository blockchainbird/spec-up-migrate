...
async function fetchSpecUpTBoilerplate() {
  ...
  try {
    ...
    // Fallback to minimal default configuration matching remote boilerplate structure
    return {
      "specs": [
        {
...
async function updateSpecsJson(directory, dryRun) {
  ...
  try {
    ...
    specsData.specs = specsData.specs.map(spec => {
      ...
      // Add author and description from boilerplate if not present at spec level
      if (!updatedSpec.author) {
        if (boilerplateSpec && boilerplateSpec.author) {
          updatedSpec.author = boilerplateSpec.author;
          changes.push(`Added author from Spec-Up-T boilerplate for ${updatedSpec.title || 'spec'}`);
        } else {
          updatedSpec.author = "Trust over IP Foundation";
          changes.push(`Added fallback author for ${updatedSpec.title || 'spec'}`);
        }
      }
      
      if (!updatedSpec.description) {
        if (boilerplateSpec && boilerplateSpec.description) {
          updatedSpec.description = boilerplateSpec.description;
          changes.push(`Added description from Spec-Up-T boilerplate for ${updatedSpec.title || 'spec'}`);
        } else {
          updatedSpec.description = "Create technical specifications in markdown. Based on the original Spec-Up, extended with Terminology tooling";
          changes.push(`Added fallback description for ${updatedSpec.title || 'spec'}`);
        }
      }
